...
//show all Legal values on click
...
//XXX this was the piece we were missing
			hintRow = -1;
			hintCol = -1;
...
// Maybe I should have used JLabel instead of JButton?
					...
					setText(row, col, "_");
					if (showLegalValues) {
						Collection<Integer> legals =

								sudoku.getLegalValues(row, col);
...
// basically, immediately create a class that implement actionlistener
...
// Integer returnValue = jfc.showSaveDialog(null);
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					JOptionPane.showMessageDialog(null, "Saved game to file " +
...
//
				...
				for (int r = 0; r < 9; r++) {
					for (int c = 0; c < 9; c++) {
						if (sudoku.isBlank(r, c) &&
								...
								sudoku.getLegalValues(r, c).size() == 1) {
		...
		JMenu check = new JMenu("Check"); //`-- New feature
		menuBar.add(check);
		addToMenu(check, "Check", new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				for (int r = 0; r < 9; r++) {
					for (int c = 0; c < 9; c++) {
						if (sudoku.isBlank(r, c) && sudoku.getLegalValues(r, c).size() == 0) {
							JOptionPane.showMessageDialog(null, "No possible value for "+ "row: "+ r +" ,and col: "+ c);
							return;
						}
					}
				}
				JOptionPane.showMessageDialog(null, "The current values don't lead to impossible solution");
			}
		});
	...
	private void createKeyboardHandlers() {
		for (int r = 0; r < buttons.length; r++) {
			for (int c = 0; c < buttons[r].length; c++) {
				buttons[r][c].addKeyListener(new MyKeyListener(r, c,
...
// This sets up 81 JButtons (9 rows * 9 columns)
		for (int r = 0; r < numRows; r++) {
			for (int c = 0; c < numCols; c++) {
				...
				b.setPreferredSize(new Dimension(squareSize,
...
// sudoku boards use 3x3 sub-grids
				...
				b.setBorder(BorderFactory.createMatteBorder(top, left,
